...
public class ReportFacade {
    ...
    /**
     * 신고 접수. 대상 존재 확인과 자기 콘텐츠 판정을 거쳐 ReportService에 넘긴다.
     *
     * 대상 도메인(board, board_comment, vintage_comment)을 알아야 하는
     ...
     *
     * 비어 있는 Optional은 "대상이 없다"가 아니라 "작성자가 없다"는 뜻이다.
     * vintage_comment는 작성자 탈퇴 시 member_id가 null이 되기 때문이다. 이 경우
     * 댓글 자체는 남아 있으므로 신고할 수 있어야 한다.
     *
     * 대상이 3종뿐이라 switch로 둔다. 늘어나면 ReportTargetType에 확인 전략을
     * 붙이는 형태가 자연스럽다.
     *
     * 존재 확인만 필요한 자리에 findById로 엔티티 전체를 읽는 것은, 작성자 ID를 함께
