 ...
 *
 * 컨테이너는 {@code static}이라 JVM 당 한 벌만 뜨고 모든 통합 테스트가 공유한다.
 *
 * 왜 {@code @Import}가 아니라 상속인가
 *   - {@code @DynamicPropertySource}는 테스트 클래스와 그 상위 클래스에서만 처리된다.
 *       {@code @Import}된 {@code @TestConfiguration}에 두면 조용히 무시되어, 컨테이너는 떠 있는데
 *       정작 테스트는 클래스패스의 임베디드 H2에 붙는다.
 *   - {@code @ServiceConnection}은 {@code @Import}로도 동작하지만 스프링 부트의 자동 설정을 전제로 한다.
 ...
 * 주입하는 이 방식을 쓴다.
 *
 * 실행에는 Docker 가 필요하다. 그 외 설정은 커밋된 {@code src/test/resources/application-test.yml}에 있어
